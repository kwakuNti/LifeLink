...
if (empty($bloodType) || $initAge <= 0 || $bmiTcr <= 0 || $dayswaitAlloc < 0) {
    ...
    header("Location: ../templates/liver_donor_info?status=error&message=Missing+or+invalid+fields");
    exit();
}
...
if ($recordExistsInBlockchain) {
    header("Location: ../templates/liver_donor_info?status=warning&message=Medical+info+already+on+blockchain");
    exit();
}
...
try {
    ...
    header("Location: ../templates/liver_donor_info?status=error&message=Database+error");
    exit();
}
...
if (file_exists($invokeScript) && is_executable($invokeScript)) {
    ...
    if ($ret2 === 0) {
        ...
        header("Location: ../templates/liver_donor_info?status=error&message=Blockchain+invoke+failed");
    }
} else {
    header("Location: ../templates/liver_donor_info?status=error&message=Invoke+script+missing");
}
exit();
